...
class ADAuthStore(Component):
    ...
    dir_scope = Option('account-manager', 'dir_scope', 1,
    ...
    dir_timeout = Option('account-manager', 'dir_timeout', 5,
    ...
    group_attr = Option('account-manager', 'group_attr', 'cn',
        "Attribute of the name of the group")

    group_validusers = Option('account-manager', 'group_validusers', None,
        "DN of group containing valid users. If None, any AD user is valid")

    group_tracadmin = Option('account-manager', 'group_tracadmin', None,
        "DN of group containing TRAC_ADMIN users (can also assign "
        "TRAC_ADMIN to an LDAP group.)")

    group_expand = Option('account-manager', 'group_expand', 1,
        "binary: expand ldap_groups into trac groups.")

    group_member_attr = Option('account-manager', 'group_member_attr', 'member',
        "which group attribute to check for members")

    group_member_value = Option('account-manager', 'group_member_value', 'dn',
    ...
    def __init__(self, ldap=None):
        self._ldap = ldap  # cache my ldap handle
        self._cache = {}  # have a memory cache
    ...
    def has_user(self, user):
        has_user = self._cache_get('hasuser: %s' % user)
       
        if has_user:
            return has_user
       
        users = self.get_users()
        has_user = user.lower() in users
        self._cache_set('hasuser: %s' % user, has_user)
        return has_user

    def check_password(self, user, password):
        """Checks the password against LDAP."""

        # Clear the caches and update session attributes.
        success = None
        ...
        filter_ = "(&(%s=%s)(objectClass=person))" % (self.user_attr, user)
        users = self._dir_search(self.dir_basedn, self.dir_scope, filter_,
                                 attrs, NOCACHE)
        
        if not users:
            raise TracError("Authenticated, but didn't find the user with "
                            "filter : %s (%s)" % (filter_, users))
        ...
        self.get_permission_groups(user, NOCACHE)
        self.get_users(NOCACHE)
    ...
    def get_permission_action(self, username):
        ...
        if self.group_tracadmin:
            users = \
                [u[0] for u in self._expand_group_users(self.group_tracadmin)]
    ...
    def get_permission_groups(self, username, use_cache=1):
        ...
        if not self.group_member_attr or self.group_member_attr == 'dn': 
            ...
            filter_ = "(&(%s=%s)(objectClass=person))" \
                      % (self.user_attr, username)
            users = self._dir_search(self.dir_basedn, self.dir_scope, filter_,
                                     [self.group_member_dn.encode('ascii')],
                                     NOCACHE)
        ...
        if group_value: 
            ...
            if groups:
                self.log.debug("%s has LDAP groups: %s",
                               username, ','.join(groups))
            else:
                self.log.debug("username %s (%s) has no LDAP groups",
                               username, dn)
    ...
    def _expand_group_users(self, group_dn):
        ...
        if self.group_expand:
            ...
            group_filter = '(%s)' % (group_dn.split(',')[0])
            g = self._dir_search(base_dn, self.dir_scope, group_filter,
                                 [group_member_attr])
            if g and group_member_attr in g[0][1]:
                users = []
                for m in g[0][1][group_member_attr]:
                    m_filter = '(%s)' % (m.split(',')[0])
                    e = self._cache_get(m_filter)
                    if not e:
                        e = self._dir_search(base_dn, self.dir_scope, m_filter)
                    ...
                    if e:
                        ...
                        group_user = e[0][0].decode(self.dir_charset)
                        users.extend(self._expand_group_users(group_user))
                    else:
                        self.log.debug("The group member (%s) is neither a "
                                       "group nor a person", e[0][0])
            else:
                self.log.debug("Unable to find user listed in group: %s", g)
                self.log.debug("This is very strange and you should probably "
                               "check the consistency of your LDAP "
                               "directory.", g)
    ...
    def _bind_dir(self, user_dn=None, passwd=None):
        ...
        if self.dir_binddn:
            ...
            self.log.debug("bind_dir: attempting general bind to %s "
    ...
    def _get_user_groups(self, attr, value):
        ...
        groups = []
        group_name_attr = self.group_attr.encode('ascii') or \
                          self.user_attr.encode('ascii')
        dir_groups = self._dir_search(basedn, self.dir_scope,
                                      '(&(%s=%s)(objectClass=group))'
                                      % (attr, value),
                                      [group_name_attr])
        ...
        if dir_groups: 
            for group in dir_groups:
                tgroup = GROUP_PREFIX + \
                         group[1][group_name_attr][0].lower().replace(' ', '_')
    ...
    def _populate_user_session(self, userinfo):
        ...
        # TODO need to have it updated by the get_dn stuff long term so
        # the db matches the auth source.
        cur = db.cursor()
        try:
            cur.execute("""
        ...
        if displayname:
            ...
            self.log.info("updating user session displayname info for "
                          "%s (%s)", uname, to_unicode(displayname))
        db.commit()
        return db.close()
    ...
    def _cache_set(self, key=None, data=None, cache_time=None):
        ...
        old_keys = cache_keys[:(self.cache_memprune * self.cache_memsize) / 100]
        for k in old_keys:
            del self._cache[k]
    ...
    def _dir_search(self, basedn=None, scope=1, filter_=None, attrs=None,
                    check_cache=1):
        ...
        key = ','.join([basedn, str(scope), filter_, ':'.join(attrs)])
        ...
        self.log.debug("_dir_search: starting LDAP search of %s %s using %s "
                       "for %s", self.dir_uri, basedn, filter_, attrs)

        res = []
        try:
            dir_charset = basedn.encode(self.dir_charset)
            res = dir_.search_s(dir_charset, scope, filter_, attrs)
        except ldap.LDAPError, e:
        ...
        try:
            cur = db.cursor()
            cur.execute("""
              INSERT OR REPLACE INTO ad_cache (id, lut, data)
              VALUES (%s, %s, %s)
              """, (key, current_time, res_str))
            db.commit()
    ...
    @staticmethod
    def _decode_list(list_in=None):
        if not list_in:
            return list_in
        list_out = []
        for val in list_in:
            list_out.append(val.encode('ascii', 'ignore'))
        return list_out
