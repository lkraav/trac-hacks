...
def collect_changes(repos, config, db=None):
    ...
    try:
        if config.branch == 'master':
            node = repos.get_node('/', repos.git.head())
        else:
            rev = None
        ...
        env.log.warn('Error accessing branch %r for configuration %r',
                    config.branch, config.name, exc_info=True)
        return
