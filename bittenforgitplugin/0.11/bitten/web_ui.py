...
class BuildConfigController(Component):
    ...
    def _render_overview(self, req):
        ...
        for config in BuildConfig.select(self.env, include_inactive=show_all):
            ...
            config_data = {
                ...
                'active': config.active, 'branch': config.branch,
    ...
    def _render_config(self, req, config_name):
        ...
        data['config'] = {
            'name': config.name, 'label': config.label, 'branch': config.branch,
...
class BuildController(Component):
    ...
    def get_timeline_events(self, req, start, stop, filters):
        ...
        db = self.env.get_db_cnx()
        cursor = db.cursor()
        cursor.execute("SELECT b.id,b.config,c.label,c.branch, b.rev,p.name,"
        ...
        for id_, config, label, branch, rev, platform, stopped, status in cursor:
            if not repos.authz.has_permission(branch):
                continue
            errors = []
