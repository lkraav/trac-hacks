...
class ChildTicketTreeMacro(WikiMacroBase):
    """This macro provides a complete hierarchy of all childtickets under
    the given ticket.
    ...
    def expand_macro(self, formatter, name, text):
        ...
        try:
            ...
            raise TracError('No ticket id supplied or it is not valid '
                            'integer value.')
        ...
        for child, parent in self.env.db_query("""
            ...
            if parent and re.match('#\d+', parent):
                self.childtickets.setdefault(int(parent.lstrip('#')), []) \
                                 .append(child)
        ...
        def ticket_anchor(t):
            title = "%s : %s : %s" % (t['type'], t['owner'], t['status'])
            return tag.a('#%s' % t.id, class_=t['status'],
                         href=req.href.ticket(int(t.id)), title=title)

        def_list = tag.dl(
            [(tag.dt(ticket_anchor(t),
                     style='padding-left: %spx;' % (int(t['indent']) * 20)),
              tag.dd("%s" % t['summary'])) for t in self.tickets],
            class_='wiki compact')
