...
def post_doxyfile(req, doxygen, doxygen_args, doxyfile, input, base_path,
                  log):
    """
    ...
    if not os.path.isdir(path_trac) or not os.access(path_trac, os.W_OK):
        ...
        env = apply_doxyfile(req.args, doxygen, doxygen_args, doxyfile, input,
                             path_trac)
...
def init_doxyfile(env, doxygen, doxyfile, input, base_path, default_doc, log):
    ...
    if not env and not os.path.isdir(path_trac) or not os.access(path_trac,
                                                                 os.W_OK):
    ...
    if os.path.exists(doxyfile):
        old = analyze_doxyfile(base_path, default_doc, input, doxyfile, {},
                               log)
    else:
        old = {}
...
def apply_doxyfile(req_args, doxygen, doxygen_args, doxyfile, input,
                   path_trac):
    """
    Save the POST data in the Doxyfile, and execute doxygen from the INPUT 
    dir, since the EXCLUDE option is computed relative to the execution path.
    ...
    dir_ = req_args.get('INPUT') if req_args.get('INPUT') else input
    if not os.path.isdir(dir_) or not os.access(dir_, os.R_OK):
        return {'msg': 'Error: ' + dir_ + ' not R_OK', 'trace': '',
                'options': i}
    p = Popen([doxygen, arg], bufsize=-1, stdout=o, stderr=e,
              cwd=dir_ if dir_ else None)
    p.communicate()
    n = p.returncode
    o.close()
    e.close()
    if n == 0:
        Popen(['chmod', '-R', 'g+w', path_trac])
        msg = ""
...
def analyze_doxyfile(base_path, default_doc, input, path, old, log):
    """
    Read a Doxyfile and build a Web form allowing to change its default 
    values. Text blocs between '#----' introduce new section.
    Other text blocs are treated as the "label" tag of the "input" tag
    described by the line matching NAME=VALUE that follows.
    Values different form default value for a field have a different CSS 
    class. For some NAMES, value cannot be choosen freely.
    """
    ...
    if c:
        log.debug('taking "%s" last characters out of %s in %s',
                  len(c.group(1)), len(content), path)
        content = c.group(1)

    m = re.compile(
        r'\s*(#-+\s+#(.*?)\s+#-+)?(.*?)$\s*([A-Z][A-Z0-9_-]+)\s*=([^#]*?)$',
        re.S | re.M)
    s = m.findall(content)
    log.debug('Found "%s" options in Doxyfile', len(s))
    options = OrderedDict()
    for o in s:
        u, explain, label, id_, value = o
        atclass = default = ''
        if id_ in old and value != old[id_]['value']:
            value = old[id_]['value']
            atclass = 'changed'
        # required for plugin to work
        if id_ == 'SERVER_BASED_SEARCH' or id_ == 'EXTERNAL_SEARCH':
            value = 'YES'
            atclass = 'changed'
        if id_ == 'OUTPUT_DIRECTORY' and base_path:
            ...
            if value:
                ...
                value = default_doc
        elif id_ == 'INPUT' and input:
            default = input + ('' if input[-1] == '/' else '/')
            value = value[len(default):]
            if value:
                atclass = 'changed'
        elif id_ == 'STRIP_FROM_PATH' and input and not value:
            value = input
            atclass = 'changed'
        elif id_ == 'PROJECT_NAME' and re.match('\s*"My Project"', value):
