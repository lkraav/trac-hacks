...
class BulkFetchTicket(Ticket):
    ...
    def select(cls, env, tkt_ids):
        with env.db_query as db:
            return cls._select_db(db, env, tkt_ids)

    @classmethod
    def _select_db(cls, db, env, tkt_ids):
        if not tkt_ids:
            return {}
...
class ExcelTicketModule(Component):
    ...
    def _convert_query_db(self, db, req, query, sheet_query, sheet_history):
    ...
    def post_process_request(self, req, template, data, content_type):
        if template == 'report_view.html' and req.args.get('id'):
            ...
            if format in ('xlsx', 'xls'):
                ...
                data['context'] = web_context(req, resource, absurls=True)
