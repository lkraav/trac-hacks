...
class FullBlogCore(Component):
    ...
    def get_bloginfotext(self):
        ...
        for value, in self.env.db_query("""
                SELECT value from system WHERE name='fullblog_infotext'
                """):
            return value
    ...
    def set_bloginfotext(self, text=''):
        ...
        self.env.db_transaction("""
            UPDATE system set value=%s WHERE name='fullblog_infotext'
            """, (text,))

    def get_prev_next_posts(self, perm, post_name):
        """Returns the name of the next and previous posts when compared
        with input 'post_name'.
        """
        prev = next = marker = ''
        found = False
    ...
    def get_months_authors_categories(self, from_dt=None, to_dt=None,
        ...
        * Note also that it only fetches from most recent version. """
        ...
        for post in blog_posts:
            ...
            total += 1
        return ([(m, m_dict.get(m, 0)) for m in sorted(m_dict, reverse=True)],
                [(a, a_dict.get(a, 0)) for a in sorted(a_dict)],
                [(c, c_dict.get(c, 0)) for c in sorted(c_dict)],
                total)
    ...
    def _get_default_postname(self, user=''):
        ...
        try:
            ...
            name = time.strftime(opt, now)
            name = name.replace('$USER', user)
        except:
            self.log.debug("FullBlog: Error parsing default_postname "
                           "option: %s", opt)
            return ''
        else:
            return name
