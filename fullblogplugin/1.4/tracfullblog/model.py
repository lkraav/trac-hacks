...
def get_blog_posts(env, category='', author='', from_dt=None, to_dt=None,
    ...
    if all_versions:
        ...
        join_operation = ""
    with env.db_query as db:
        clauses = [category and ("bp1.categories "+ db.like(), "%"+category+"%"),
        ...
        for row in db("""
                ...
                FROM fullblog_posts bp1 %s %s
                ORDER BY bp1.publish_time DESC
                """ % (join_operation, where_clause), args):
...
def get_blog_resources(env):
    ...
    return [blog_realm(id=post, version=0)
            for post, in env.db_query("""
                SELECT bp1.name FROM fullblog_posts bp1,
                 (SELECT name, max(version) AS ver
                 FROM fullblog_posts GROUP BY name) bp2
                WHERE bp1.name = bp2.name AND bp1.version = ver
                ORDER BY bp1.publish_time DESC
                """)]
...
class BlogComment(object):
    ...
    def create(self, comment='', author='', verify_only=False):
        ...
        self.log.debug("Creating blog comment number %d for %r",
                       number, self.post_name)
        self.env.db_transaction("""
            INSERT INTO fullblog_comments VALUES (%s,%s,%s,%s,%s)
            """, (self.post_name, number, comment, author,
    ...
    def delete(self):
        if not self.post_name or not self.number:
            return False
        self.log.debug("Deleting blog comment number %d for %r",
                       self.number, self.post_name)
        self.env.db_transaction("""
            DELETE FROM fullblog_comments WHERE name=%s AND number=%s
    ...
    def _load_comment(self, number):
        ...
        self.log.debug("Fetching blog comment number %d for %r",
                       number, self.post_name)
        for comment, author, time in self.env.db_query("""
                SELECT comment, author, time FROM fullblog_comments
                WHERE name=%s AND number=%s
                """, (self.post_name, number)):
            self.number = number
            self.comment = comment
            self.author = author
            self.time = to_datetime(time, utc)
            return True
        return False
    ...
    def _next_comment_number(self):
        ...
        comments = sorted(n for n, in self.env.db_query("""
            SELECT number FROM fullblog_comments WHERE name=%s
            """, (self.post_name,)))
        if comments:
...
class BlogPost(object):
    ...
    def __init__(self, env, name, version=0):
        ...
        for prop in self._db_default_fields:
    ...
    def save(self, version_author, version_comment=u'', verify_only=False):
        ...
        self.log.debug("Saving new version %d of blog post %r "
        ...
        self.env.db_transaction("""
            ...
            """, (self.name, version, self.title, self.body,
                  ...
                  version_comment, version_author, self.author,
                  self.categories))
        self._load_post(version)
        return warnings
    ...
    def get_versions(self):
        ...
        return [v for v, in self.env.db_query("""
            SELECT version from fullblog_posts WHERE name=%s
            """, (self.name,))]
    ...
    def _fetch_fields(self, version=0):
        ...
        for row in self.env.db_query("""
                SELECT title,body,publish_time,version_time,version_comment,
                 version_author,author,categories
                FROM fullblog_posts
