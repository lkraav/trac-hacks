...
class FullBlogModule(Component):
    ...
    month_names = ListOption('fullblog', 'month_names',
        doc="""Ability to specify a list of month names for display
    ...
    personal_blog = BoolOption('fullblog', 'personal_blog', False,
        """When using the Blog as a personal blog (only one author),
        setting to 'True' will disable the display of 'Browse by author:'
        in sidebar, and also removes various author links and references.
        """)
    ...
    def _parse_path(self, req):
        ...
        if not path_items:
            pass # empty default for return is fine
        elif len(path_items) > 1 and \
                path_items[0].lower() in ['view', 'edit', 'delete']:
            ...
            command = path_items[0].lower()
            pagename = req.args.get('name','') or \
                    (len(path_items) > 1 and '/'.join(path_items[1:]))
        elif len(path_items) > 1 and \
                path_items[0].lower() in ['author', 'category']:
