...
class HackInstaller(object):
    ...
    def __init__(self, env, url, builddir=None, version=None, user=None,
                 password=None):
        self.url = url
        if not builddir:
        ...
        if not version:
            ...
            if md:
                ...
                raise TracError('HackInstaller is unable to determine what '
                                'version of Trac you are using, please '
    ...
    def install_hack(self, name, rev, new=False):
        ...
        self.log.info('Installing %s hack.', name)
    ...
    def install_plugin(self, name, rev, new=False):
        ...
        temp_file = tempfile.NamedTemporaryFile(
            suffix='.txt', prefix='hackinstall-record',
            dir=self.builddir, mode='r')
        command = "easy_install -m --install-dir=%s --record=%s %s/svn/%s/%s" \
                  % (plugins_dir, temp_file.name, self.url, name.lower(),
                     self.version)
        self.log.info('Running os.system(%s)', command)
        rv = os.system(command)
        if rv != 0:
            self.log.warning(
                "easy_install failed with a return code of %s. Please look "
                "in your webserver's error log for the output", rv)
            return (False, None)
        ...
        for old_file in installed:
            ...
            self.log.debug("Processing file '%s'", old_file)
            if old_file.endswith('.egg'):
                dist = pkg_resources.Distribution.from_filename(
                    old_file,
                    pkg_resources.EggMetadata(zipimport.zipimporter(old_file)))
                ...
                if new:
                    if dist.has_metadata('trac_plugin.txt'):
                        ...
                        for line in dist.get_metadata_lines('trac_plugin.txt'):
                            self.config.set('components', line.strip() + '.*',
                                            'disabled')
                    else:
                        self.log.debug('Entry point plugin detected')
                        for entry_name in dist.get_entry_map('trac.plugins'):
                            self.log.debug("Processing entry name '%s'",
                                           entry_name)
                            entry_point = \
                                dist.get_entry_info('trac.plugins', entry_name)
                            self.log.debug("Module name is '%s'",
                                           entry_point.module_name)
                            self.config.set('components',
                                            entry_point.module_name + '.*',
                                            'disabled')
                    self.config.save()
                ...
                new_name = None
                # Rename a plugin from Foo-0.5-py2.4.egg to
                # Foo-0.5-rREV-py2.4.egg
                ...
                if md:
                    ...
                    if md2:
                        ...
                        new_name = '%s%s_r%s%s' % (
                            md.group(1), md.group(2), rev, md.group(3))
                    new_file = os.path.join(plugins_dir, new_name)
                    self.log.debug('Renaming %s to %s', old_file, new_file)
                ...
                if md:
                    for plugin_name in os.listdir(plugins_dir):
                        ...
                        self.log.debug(
                            "Removal scan: new_name='%s' plugin_name='%s'",
                            new_name, plugin_name)
                        if md2 and new_name != plugin_name and \
                                md.group(1) == md2.group(1):
                            plugin_file = os.path.join(
                                plugins_dir, plugin_name)
                            self.log.debug('Removing %s', plugin_file)
    ...
    def download_hack(self, name, rev):
        ...
        for zipped in zip.namelist():
            ...
            if not os.path.exists(os.path.dirname(zippedpath)):
                self.log.debug("Trying to make directory '%s'",
                               os.path.dirname(zippedpath))
