...
from core import HackInstaller
...
class HackInstallPlugin(Component):
    ...
    implements(IAdminPageProvider, IEnvironmentSetupParticipant,
               IPermissionRequestor, ITemplateProvider)

    def __init__(self):
        """Perform basic initializations."""
        url = self.config.get('hackinstall', 'url',
                              default='http://trac-hacks.org')
    ...
    def process_admin_request(self, req, cat, page, path_info):
            ...
            if page == 'general':
                if 'save_settings' in req.args:
                    ...
                    if req.args.get('url') != self.installer.url:
                        self.config.set('hackinstall', 'url',
                                        req.args.get('url'))
                        changes = True

                    # Process the version fields
                    if 'override_version' in req.args:
                        self.config.set('hackinstall', 'version',
                    ...
                    selected = [x[9:] for x in req.args
                                if x.startswith('doupdate_')]
                    if selected:
                        self._install_plugins(*selected)

            elif page == 'plugins':
                installs = [k[8:] for k in req.args
                            if k.startswith('install_')]
                if installs:
                    hack = installs[0]
                    req.hdf['hackinstall.message'] = "Installing plugin %s" % (
                        hack)
        ...
        template = {
            'general': 'hackinstall_admin.cs',
            'plugins': 'hackinstall_admin_plugin.cs',
            'macros': 'hackinstall_admin_macro.cs'
        }[page]
    ...
    def _get_hacks(self, type):
        ...
        cursor = db.cursor()
        cursor.execute(
            'SELECT distname, name FROM hacks WHERE distname NOTNULL')
        ...
        if type == 'plugin':
            for f in os.listdir(self.env.path + '/plugins'):
                ...
                md = re.match('([^-]+)-([^-]+)-', f)
                if md:
                    plugin = distnamemap.get(
                        md.group(1), md.group(1) + 'plugin').lower()
                    md2 = re.search('r(\d+)', md.group(2))
                    if md2:
                        installed[plugin] = int(md2.group(1))
                    else:
                        installed[plugin] = 0
                    self.log.debug('Extracted version = %s' %
                                   installed[plugin])
        elif type == 'macro':
        ...
        hacks = {}
        cursor.execute("""
            SELECT id, name, current, description, deps
            FROM hacks WHERE name LIKE '%%%s'
            """ % type.title())
        for row in cursor:
            hacks[row[1]] = {
                'id': row[0],
                'current': int(row[2]),
                'installed': installed.get(row[1].lower(), -1),
                'description': row[3],
                'deps': row[4],
                'lowername': row[1].lower()
            }
        return hacks
    ...
    def _check_version(self):
        ...
        if self.installer.version not in versions:
            raise TracError("Trac-Hacks doesn't know about your version of "
                            "Trac (%s)" % self.version)
        return True
    ...
    def _update(self, type):
        ...
        for hack in hacks:
            ...
            if row:
                cursor.execute("""
                    UPDATE hacks
                    SET current = %s, description = %s, deps = %s
                    WHERE name = %s
                    """, (hack[1], details['description'], deps, hack[0]))
            else:
                cursor.execute("""
                    INSERT INTO hacks (name, current, description, deps)
                    VALUES (%s, %s, %s, %s)
                    """, (hack[0], hack[1], details['description'], deps))
        db.commit()
    ...
    def _pending_updates(self):
        ...
        for hack, params in self.plugins.iteritems():
            if params['current'] > 0 and params['installed'] >= 0 and \
                    params['current'] > params['installed']:
        ...
        for plugin in plugins:
            install_ok, ret = \
                self.installer.install_plugin(
                    plugin, self.plugins[plugin]['current'],
                    self.plugins[plugin]['installed'] == -1)
            if install_ok:
                distname = ret[0]
                cursor.execute("""
                    UPDATE hacks SET distname = %s WHERE name = %s
                    """, (distname, plugin))
            else:
                self.log.warning(
                    "Encountered an unknown error when installing %s. Please "
                    "see your webserver's logs for more information.", plugin)
                raise TracError("Error during installation, please see the "
