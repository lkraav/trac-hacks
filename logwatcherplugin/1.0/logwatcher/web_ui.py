...
class LogViewerPage(Component):
    ...
    def get_admin_panels(self, req):
        if req.perm.has_permission('TRAC_ADMIN') or req.perm.has_permission("LOG_WATCHER_VIEW"):
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        try:
          log_list = self.api.get_logfile_names(self._log_destinations, self._log_levels, sort_by)
...
#          if req.method == "POST":
          if not autoload:
            ...
            data['level'] = int(self.env.config.get('logwatcher', 'autolevel') or 3)
            data['up'] = int(self.env.config.getbool('logwatcher', 'autoup') or True)
            data['invert'] = 0
            data['regexp'] = 0
            data['filter'] = ''
            data['tail'] = self.env.config.get('logwatcher', 'autotail') or ''
            self._do_process(data, logfile)
          else:
            data['level'] = int(self.env.config.get('logwatcher', 'defaultlevel') or 3)
            data['up'] = int(self.env.config.getbool('logwatcher', 'defaultup') or True)
            data['invert'] = 0
            data['regexp'] = 0
            data['filter'] = ''
            data['tail'] = self.env.config.get('logwatcher', 'defaulttail') or ''
