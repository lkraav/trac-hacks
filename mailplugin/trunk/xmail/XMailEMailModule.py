...
class XMailEventHandler(Component):
    ...
    def filter_stream(self, req, method, filename, stream, data):
        if not self._TimerIsStillAlive():
            ...
            if not isinstance(preferred, list):
                preferred = [preferred]
            self._locale_string = Locale.negotiate(
                preferred, available, sep='-')
    ...
    def _TimerIsStillAlive(self):
        ...
        for threadOb in threadList:
            # if threadOb.__class__.__dict__.has_key('name') and threadOb.name
            # == "xmailThread":
            if threadOb.getName() == "xmailThread":
                if threadOb.isAlive() == False:
                    self._createXMailThread()
                    self.log.warn(
                        '_TimerIsStillAlive: Thread has not been alive; started new Thread')
                    return False
                return True  # return since it has already a Thread
        self.log.info(
            "[_TimerIsStillAlive] have not found any thread named 'xmailThread'")
#        self.log.debug('[_TimerIsStillAlive] started Thread')
        self._createXMailThread()
        return False

    def _createXMailThread(self):
        myThread = XMailTimerThread(
            self._sendAllMails, self.DEFAULT_SLEEP_TIME, self.log)
...
#=========================================================================
    def _sendAllMails(self):
        ...
        for filter_id, username in filterids:
            self.log.info(
                "[XMail._sendAllMails] -- filter_id: %s -- username: %s" % (filter_id, username))
            tickets = {}
            ...
            changed_tickets = self._get_relevant_tickets(
                'time != changetime and changetime', filter)
            ...
            if len(tickets) == 0:
                if filter.values['interval'] > 0:
                    ...
                    self.log.debug(
                        "[XMail._sendAllMails] no relevant tickets, since length of tickets is %s" % len(tickets))
                # nothing to do, so continue with next filter
                continue

            user_data = self._get_user_data(username)
            self.log.debug("user data: %r -- negotiated: %s" %
                           (user_data, self._locale_string))
            if not user_data.has_key('language'):
                self.log.debug(
                    "got no user-specific language, so using locale %s" % self._locale_string)
            ...
            self._set_next_exe(filter, self._get_current_time())
            self.log.info("[XMail._sendAllMails] XMailTicketNotify: sent email with %r tickets for user %s" % (
                len(tickets), username))
        return
    ...
    def _get_relevant_tickets(self, timefilter, filter):
        ...
        if filter.values['interval'] > 0:
            t = filter.values['nextexe'] - \
                (filter.values['interval'] * 1000000)
        else:
...
#            print "result: %r" % result
            ...
            self.log.info(
                "could not get relevant ticket, since error occured: %s\n sql: %s" % (e, sql))
    ...
    def _set_next_exe(self, filter, current_time):
        ...
        if filter.values['interval'] > 0:
            next_exe = (
                filter.values['interval'] * SEC_MULTIPLIER) + filter.values['nextexe']

        sql = "update xmail set nextexe=%i, lastsuccessexe=%i where id=%i" % (
            next_exe, current_time, filter.id)
...
#    _locale_string = 'en'
    ...
    def __init__(self, env, data, template_name=None):
        ...
        if self._data and self._data['user_data']:
            # self._locale_string = self._data['user_data']['language'] # not
            # used at the moment
            self._email_adr = self._data['user_data']['email']
# print "[XMailMultiTicketNotify] self._data: %r --- data: %r --- tickets:
# %r" % (self._data, data, data['new_tickets'])
    ...
    def get_recipients(self, tktid):
        ...
        self.data['new_tickets'] = self._format_tickets(
            self._data['new_tickets'])
        self.data['changed_tickets_count'] = len(self._data['changed_tickets'])
        self.data['changed_tickets'] = self._format_tickets(
            self._data['changed_tickets'])
        ...
        link = "%s/xmail/xmail-edit.html?id=%i" % (
            self.env.abs_href.base, filter.id)
        self.data['change_hint'] = tag_(
            'To change the filter go to %(link)s', link=link)
...
class XMailTimerThread(Thread):
    ...
    def run(self):
        while True:
            try:
                # error occured: 'ascii' codec can't encode character u'\xfc'
