...
from trac.util.datefmt import parse_date, to_utimestamp, format_datetime
import math

# =========================================================================
# idea:
# maybe use a view for abstraction;
# then it is not needed to regex about SQL-Where clause
# =========================================================================

# xmail table object
# see trac/db_default.py for samples and trac/db/schema.py for
...
class FilterObject(object):
    ...
    def check_filter(self):
        ...
        for field in self.must_fields:
            is_ok &= self.values[field] is not None and len(
                self.values[field]) > 0
        return is_ok
...
#        return format_datetime(time_in_sec, format='%d.%m.%Y %H:%M:%S')
            ...
            with env.db_query as db:
                ...
                for i, col in enumerate(XMAIL_TABLE.columns):
                    if col.name == "selectfields":
                        self.select_fields = self.get_select_fields_list(
                            result[i])
                    elif col.name == "filtername":
                        self.name = result[i]

                    if col.name == "whereclause":
                        self.values[col.name] = self._replace_sql_where_clause(
                            result[i], True)
                    else:
    ...
    def get_filter_select(self, selectfields=None, whereclause=None, additional_where_clause=None):
        ...
        if additional_where_clause:
            whereclause = "(%s) and (%s)" % (
                additional_where_clause, whereclause)
        ...
        try:
            with env.db_query as db:
                ...
                if sqlResult and len(sqlResult) > 0:
                    ...
                    if len(sqlResult) > 2:
                        sql_filter = self.get_filter_select(
                            sqlResult[2],  sqlResult[3])

        except Exception, e:
            raise Exception(
                e, "Error executing SQL Statement \n ( %s ) " % (sql))
        return sql_filter
    ...
    def save(self, env, update=False, id=None):
        ...
        if update and not id:
            raise Exception(
                "NO SQL", "ERROR: when updating field 'ID' is required.")
        elif update:
            sql += "UPDATE %s SET " % XMAIL_TABLE.name
        else:
            sql = "INSERT INTO %s (%s) VALUES (" % (
                XMAIL_TABLE.name, get_col_list(['id']))
...
#                    print "nach repl: %s" % where_clause
        ...
        try:
            if sel_fields or where_clause:
                with env.db_query as db:
                    cursor = db.cursor()
                    filter_sql = self.get_filter_select(
                        sel_fields, where_clause)
                    cursor.execute(filter_sql)
            with env.db_transaction as db:
                db(sql)
        except Exception, e:
