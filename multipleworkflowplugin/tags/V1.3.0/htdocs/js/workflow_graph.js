(function($){
  ...
  function drawWorkflows(data, div) {
  ...
  window.drawWorkflows = drawWorkflows;
...
(function($) {
  ...
  //              the rendered text.
  $.fn.autoPreviewWorkflow = function(href, args, update) {
    ...
    return this.each(function() {
      ...
      function request() {
        ...
        if (!updating && (text != data["text"])) {
          ...
          data["text"] = text;
          $.ajax({
            type: "POST", url: href, data: data, dataType: "json",
...
jQuery(document).ready(function(){
    var graphdiv = $('.multiple-workflow-graph');
      var data = window['graph_' + graphdiv.attr('id').slice(-12)];
      drawWorkflows(data, graphdiv);
    }
  /*
function() {
    $('.trac-workflow-graph').each(function (index) {
