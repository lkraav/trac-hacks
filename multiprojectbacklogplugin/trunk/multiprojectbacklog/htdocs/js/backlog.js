jQuery(document).ready(function ($) {
  ...
  function drop_on_th(event, ui) {
    $('.ticket-th').on('dropout', function (event, ui) {
      $('th', this).removeClass("tr-dragover");
    });
    var drop_tid = ui.draggable.attr('id').split('_')[1];
    var before_tid = $('#tickets').find('tr:first').attr('id').split('_')[1];
    var target = this;
    /* Save value  */
    $.post(mp_post_url + '/move_before',
      {
        ...
        if (data['errorcode'] === 200) {
          ui.draggable.insertBefore($('#tickets').find('tr:first'));
          highlight_table(true);
          ui.draggable.effect('highlight', {duration: 1200});
        } else {
          highlight_table(false);
  ...
  $('.ticket-tr').on('dropover', function (event, ui) {
    $(this).addClass('tr-dragover');
  });
  $('.ticket-tr, .ticket-th').on('dropout', function (event, ui) {
    $(this).removeClass('tr-dragover');
  });

  $('.ticket-th').on('dropover', function (event, ui) {
    $('th', this).addClass("tr-dragover");
  });
  $('.ticket-th').on('dropout', function (event, ui) {
    $('th', this).removeClass("tr-dragover");
  });
  ...
  $('.milestone').on('dropover', function (event, ui) {
    $(this).addClass('milestone-dragover')
  });
  $('.milestone').on('dropout', function (event, ui) {
    $(this).removeClass('milestone-dragover')
  });
