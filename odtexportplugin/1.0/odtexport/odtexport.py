...
class ODTExportPlugin(Component):
    ...
    def wiki_to_html(self, wiki_text, req):
        ...
        for macro in self.remove_macros:
            wiki_text = re.sub('\[\[%s(\([^)]*\))?\]\]' % macro, "", wiki_text)
...
class ODTFile(object):
    ...
    def xhtml_to_odt(self, xhtml):
        ...
        if hasattr(etree.XSLT, 'strparam'):
            url = etree.XSLT.strparam(self.env.abs_href('/wiki/%s'
                                                        % self.page_name))
    ...
    def handle_chrome_img(self, img_mo):
        ...
        # Normalize the filename. Some people find it funny to create URLs
        # such as /chrome/site/../common/trac_logo_mini.png. Yes,
        # that's you WikiFormatting.
        ...
        for provider in chrome.template_providers:
            for tpdir in [os.path.normpath(htdir[1])
                          for htdir in provider.get_htdocs_dirs()
                          if htdir[0] == prefix]:
    ...
    def insert_content(self, content):
        for kw in ['replace', 'wikiversion', 'wikiname',
                   'timestamp', 'cut_start', 'cut_stop']:
        ...
        for k, v in {'wikiversion': str(self.page_version),
            ...
            for xmltype in self.xml.keys():
                if self.options[k + '_keyword'] and \
                   self.xml[xmltype].count(self.options[k + '_keyword']) > 0:
        ...
        if self.options['replace_keyword'] and \
            ...
            self.xml['content'] = \
                self.xml["content"].replace('</office:text>',
                                            content + '</office:text>')
        # Cut unwanted text
        if self.xml['content'].count(self.options['cut_start_keyword']) > 0 \
            ...
            self.xml['content'] = \
                re.sub(re.escape(self.options['cut_start_keyword'])
    ...
    def compile(self):
        ...
        if self.styles:
            ...
            self.xml['styles'] = \
                self.xml['styles'].replace('</office:styles>',
                                           '%s</office:styles>' % styles)
        # fonts
        if self.fonts:
    ...
    def save(self):
        ...
        # Create the zip file
        document = StringIO()
        new_zf = zipfile.ZipFile(document, 'w', zipfile.ZIP_DEFLATED)
        for root, dirs, files in os.walk(self.tmpdir):
            for cur_file in files:
                real_path = os.path.join(root, cur_file)
                internal_path = \
                    os.path.join(root.replace(self.tmpdir, ''), cur_file)
                new_zf.write(real_path, internal_path)
        new_zf.close()
