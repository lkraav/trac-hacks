...
#===============================================================================
    ...
    def _save_project_no(self, ticket, changed_fields):
        ...
        sql_select = "select value from ticket_custom" \
            " where ticket=%s and name='%s'" % (ticket.id, 'projectplugin')
        
        sql_update = "update ticket_custom " \
              " set value = '%s'"\
              " where ticket=%s and name='%s'" % (project_no, ticket.id, 'projectplugin')
              
        sql_insert = "insert into ticket_custom (ticket, name, value)" \
              " values(%s, '%s', '%s') " % (ticket.id, 'projectplugin', project_no)
...
#        print "data is: %s" % data
    ...
    def _get_enums(self, enum_name):
        ...
        db = self.env.get_db_cnx()
        myCursor = db.cursor()
        sql = "select type, name, value from enum where type='%s' order by value" % enum_name
        sqlResult = []
    ...
    def _get_milestones(self):
        db = self.env.get_db_cnx()
        myCursor = db.cursor()
        sql = "select name from milestone where completed=0 order by name"
        sqlResult = []
