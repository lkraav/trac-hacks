...
class ScreenshotsMatrixView(Component):
    implements(IScreenshotsRenderer, ITemplateProvider)

    # Configuration options.
    rows = IntOption('screenshots-matrix', 'rows', 3,
                     doc=_("Number of screenshot preview rows."))
    columns = IntOption('screenshots-matrix', 'columns', 3,
                        doc=_("Number of screenshot columns."))
    width = IntOption('screenshots-matrix', 'width', 160,
                      doc=_("Width of screenshot preview."))
    height = IntOption('screenshots-matrix', 'height', 120,
                       doc=_("Height of screenshot preview."))

    def __init__(self):
        # Items for not specified screenshot.
        self.null_screenshot = {
            'id': -1,
            'name': '',
            'description': ''
        }
    ...
    def render_screenshots(self, req):
        ...
        req.data['matrix'] = self._build_matrix(index,
                                                req.data['screenshots'])
