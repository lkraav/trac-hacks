...
from trac.web.chrome import add_stylesheet, format_datetime, web_context
...
{{{
 [[ScreenshotsList($name - $description ($widthx$height))]]
    ...
    def expand_macro(self, formatter, name, content):
        ...
        if name == 'Screenshot':
            ...
            try:
                ...
                raise TracError(_("Missing screenshot ID in macro "
                                  "arguments."))
            ...
            if screenshot:
                ...
                if 'width' not in attributes and 'height' in attributes:
                    attributes['width'] = \
                        int(int(attributes['height']) *
                            (float(screenshot['width']) /
                             float(screenshot['height'])) +
                            0.5)
                if 'height' not in attributes and 'width' in attributes:
                    attributes['height'] = \
                        int(int(attributes['width']) *
                            (float(screenshot['height']) /
                             float(screenshot['width'])) +
                            0.5)
                ...
                image = html.img(src=href.screenshots(screenshot['id'],
                    format='raw', width=attributes['width'],
                    height=attributes['height']), **img_attributes)
                link = html.a(image, href=href.screenshots(
                    screenshot['id'], format=attributes['format']),
                ...
                return html.a(screenshot_id, href=href.screenshots(),
            ...
            # Get all screenshots.
            screenshots = api.get_screenshots_complete()
            ...
            for screenshot in screenshots:
                ...
                list_items.append(html.li(html.a(list_item,
                    href=href.screenshots(screenshot['id']))))
    ...
    def _screenshot_link(self, formatter, ns, params, label):
        ...
        if ns == 'screenshot':
            ...
            if match:
                ...
                # Bad format of link.
                return html.a(label, href=href.screenshots(), title=params,
                              class_='missing')
    ...
    def _format_description(self, context, template, screenshot):
        description = template.replace('$id',
            to_unicode(screenshot['id']))
        ...
        description = description.replace('$time',
            format_datetime(to_datetime(screenshot['time'], utc)))
