jQuery(document).ready(function($) {
    ...
    function bind(element, events) {
        for (var type in events) {
            element.bind(type, events[type]);
        }
    }

    var compatFuncs = {'$': [], '$.fn': []};
    if (!('htmlFormat' in $)) {
        compatFuncs['$'].push('htmlFormat');
        $.htmlFormat = (function() {
            /* From 0.12-stable/trac/htdocs/js/trac.js */
            var quote = {"&": "&amp;", "<": "&lt;", ">": "&gt;",
                         '"': "&quot;"};
            return function(str) {
                function htmlEscape(value) {
                    ...
                    return value.replace(/[&<>"]/g,
                                         function(c) { return quote[c] });
                }
                var args = arguments;
                return str.replace(/\${?([1-9][0-9]*)}?/g, function(_, k) {
                    return htmlEscape(args[k - '0']);
                }); 
            }
        })();
    }
    if (!('delegate' in $.fn)) {
        compatFuncs['$.fn'].push('delegate');
        $.fn.delegate = function(selector, type, handler) {
            var node = this;
            switch (type) {
            case 'mouseenter':
            ...
            return this.bind(type, function(event) {
                ...
                node.find(selector).each(function() {
                    if (this === target || contains(this, target)) {
                        ...
                        $.each(['altKey'], function(idx, val) {
                            if (!(val in event) && val in origEvent) {
                                event[val] = origEvent[val];
                            }
                        });
    ...
    function removeCompatFuncs() {
        for (var name in compatFuncs['$']) {
            delete $[name];
        }
        for (var name in compatFuncs['$.fn']) {
            delete $.fn[name];
        }
    }
