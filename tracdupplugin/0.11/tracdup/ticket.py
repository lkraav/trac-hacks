...
class TracDupPlugin(core.Component):
    ...
    core.implements(api.ITicketChangeListener, api.ITicketManipulator)
    ...
    def _recalculate_ticket(self, ticket):
        ...
        if ticket.values.get('dups', None) == dups and \
                int(ticket.values.get('dup_count', '')) == dup_count:
            return False
...
""" % ticket_id)
    ...
    def ticket_changed(self, ticket, comment, author, old_values):
        ...
        if old_values.get('status', None) == 'closed' and \
                ticket.values.get('dup_of', None):
            self.env.log.debug("ticket_changed: duplicate ticket %s "
                               "reopened, removing dup_of", ticket.id)
        ...
        if not old_values.has_key('dup_of'):
            self.env.log.debug("No dup_of change on %s", ticket.id)
            return
        ...
        if old_values.get('dup_of', None) is None and master_id == '':
            self.env.log.debug("dup_of was added to %s without a value, "
                               "return", ticket.id)
            return
    ...
    def validate_ticket(self, req, ticket):
        ...
        if ot.values['status'] == u'closed' and \
                ticket.values['status'] == u'reopened':
            self.env.log.debug("validate_ticket: "
                "refusing to dup_of and reopen ticket #%s", ticket.id)
            res.append(('status',
