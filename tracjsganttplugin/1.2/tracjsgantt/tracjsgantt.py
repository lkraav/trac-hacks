...
from trac.util.datefmt import localtz, to_utimestamp
...
class TracJSGanttSupport(Component):
    ...
    Option('trac-jsgantt', 'option.display', None,
           """Display filter for tickets in the form
           'field1:value1|field2:value2' or 'field:value1|value2';
           displays tickets where field1==value1, etc.
           """)

    Option('trac-jsgantt', 'option.order', 'wbs',
           """Fields to sort tasks by before display. May include
           tickets fields (including custom fields) or 'wbs'.
           """)
...
All other macro arguments are treated as TracQuery specification (e.g., milestone=ms1|ms2) to control which tickets are displayed.
    ...
    def _add_sample_tasks(self):
        ...
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',1,   "Define Chart API",     "",          "",          "#ff0000", "http://help.com",      0, "Brian",     0,  1, 0,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',11,  "Chart Object",         "2/20/2011", "2/20/2011", "#ff00ff", "http://www.yahoo.com", 1, "Shlomy",  100,  0, 1,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',12,  "Task Objects",         "",          "",          "#00ff00", "",                     0, "Shlomy",   40,  1, 1,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',121, "Constructor Proc",     "2/21/2011", "3/9/2011",  "#00ffff", "http://www.yahoo.com", 0, "Brian T.", 60,  0, 12, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',122, "Task Variables",       "3/6/2011",  "3/11/2011", "#ff0000", "http://help.com",      0, "",         60,  0, 12, 1,121));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',123, "Task Functions",       "3/9/2011",  "3/29/2011", "#ff0000", "http://help.com",      0, "Anyone",   60,  0, 12, 1, 0, "This is another caption"));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',2,   "Create HTML Shell",    "3/24/2011", "3/25/2011", "#ffff00", "http://help.com",      0, "Brian",    20,  0, 0,  1,122));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',3,   "Code Javascript",      "",          "",          "#ff0000", "http://help.com",      0, "Brian",     0,  1, 0,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',31,  "Define Variables",     "2/25/2011", "3/17/2011", "#ff00ff", "http://help.com",      0, "Brian",    30,  0, 3,  1, 0,"Caption 1"));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',32,  "Calculate Chart Size", "3/15/2011", "3/24/2011", "#00ff00", "http://help.com",      0, "Shlomy",   40,  0, 3,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',33,  "Draw Taks Items",      "",          "",          "#00ff00", "http://help.com",      0, "Someone",  40,  1, 3,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',332, "Task Label Table",     "3/6/2011",  "3/11/2011", "#0000ff", "http://help.com",      0, "Brian",    60,  0, 33, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',333, "Task Scrolling Grid",  "3/9/2011",  "3/20/2011", "#0000ff", "http://help.com",      0, "Brian",    60,  0, 33, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',34,  "Draw Task Bars",       "",          "",          "#990000", "http://help.com",      0, "Anybody",  60,  1, 3,  1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',341, "Loop each Task",       "3/26/2011", "4/11/2011", "#ff0000", "http://help.com",      0, "Brian",    60,  0, 34, 1, "332,333"));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',342, "Calculate Start/Stop", "4/12/2011", "5/18/2011", "#ff6666", "http://help.com",      0, "Brian",    60,  0, 34, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',343, "Draw Task Div",        "5/13/2011", "5/17/2011", "#ff0000", "http://help.com",      0, "Brian",    60,  0, 34, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',344, "Draw Completion Div",  "5/17/2011", "6/04/2011", "#ff0000", "http://help.com",      0, "Brian",    60,  0, 34, 1));\n'
        tasks += self.GanttID+'.AddTaskItem(new JSGantt.TaskItem('+self.GanttID + \
            ',35,  "Make Updates",         "10/17/2011","12/04/2011","#f600f6", "http://help.com",      0, "Brian",    30,  0, 3,  1));\n'
        return tasks
