...
from trac.util.datefmt import format_date, localtz, to_datetime, to_utimestamp
...
class TracPM(Component):
    ...
    def environment_needs_upgrade(self, db=None):
        for row in self.env.db_query('SELECT value FROM system WHERE name=%s',
                                     (db_default.name, )):
            version = row[0]
            break
        else:
            version = 0
        try:
            self.found_db_version = int(version)
    ...
    Option(cfgSection, 'parent_format', '%s',
           """Format of ticket IDs in parent field (Only for fields.parent.
           Should be blank when configuring parent-child as a relation.)
           """)

    Option(cfgSection, 'milestone_type', '*deprecated*',
           """Ticket type for milestone-like tickets (Deprecated;
    ...
    Option(cfgSection, 'incomplete_milestone_goal_status', 'active',
           """Status to give goal-type tickets representing incomplete
           Trac milestones
           """)
    ...
    def __init__(self):
            ...
            if value != [] and value is not None:
                ...
                if len(value) != 3:
                    ...
                    for f in relations[r]:
                        if f in self.fields:
                            self.log.error('Cannot configure %s as a field '
        ...
        self.goalTicketType = self.config.get(
            self.cfgSection, 'milestone_type')
        ...
        self.hpe = float(self.config.get(
            self.cfgSection, 'hours_per_estimate'))
        ...
        self.dbDateFormat = str(self.config.get(
            self.cfgSection, 'date_format'))
            ...
            if not self.isCfg('parent'):
                ...
                if options['root'] == 'self':
                    if this_ticket:
                        ...
                        # FIXME - display an error here that self needs ticket
                        pass
                else:
        ...
        if options.get('goal'):
            if not self.isCfg('succ'):
                ...
                while nodes != nodes2:
                    ...
                    if self.isCfg('parent'):
                        nodes = nodes2 | set(
                            self._followLink(nodes2, 'parent',
                                             self.parent_format))
                    else:
                        nodes = nodes2
        ...
        if len(milestones) > 0:
            ...
            with self.env.db_query as db:
                ...
                for row in cursor:
                    ...
                    tid = tid - 1
                    milestoneTicket = \
                        self._pseudoTicket(tid, msName,
                                           'Milestone %s' % msName, msName)

                    # If completed date is set (non-0), the milestone is closed
                    if msCompletedDate:
        ...
        if ids:
            with self.env.db_query as db:
                ...
                for r in self.relations:
                    ...
                    # ... query all relations with desired IDs on either end.
                    # See explanation in _followLink()
                    inClause = "IN (%s)" % ','.join(('%s',) * len(ids))
...
class SimpleSorter(BaseSorter, Component):
    ...
    def prepareTasks(self, ticketsByID):
        ...
        for tid in ticketsByID:
            ...
            if self.prioMap.get(ticket['priority']) is None:
...
class ResourceScheduler(Component):
    ...
    def scheduleTasks(self, options, ticketsByID):
                ...
                if available >= math.fabs(hours):
                    ...
                    if available == 0:
                        if sign == -1:
                            ...
                            delta += \
                                timedelta(hours=options['hoursPerDay'] - h)
        ...
        def _schedule_task(t, ancestorLimit, dependentLimit,
            ...
            if t.get('_calc_' + fromField) is None:
            ...
            # task.
            if t.get('_calc_' + toField) is None:
                # Use actual dates, if requested.
            ...
            if _betterDate(t['_calc_' + toField], t['_calc_' + fromField]):
                ...
                self._logSch('%s was %s' %
                             (fromField, t['_calc_' + fromField]))
                hours = self.pm.workHours(t)
                t['_calc_' + fromField][0] = t['_calc_' + toField][0] + \
                    _calendarOffset(t,
                                    (-1 / dir) * hours,
                                    t['_calc_' + toField][0])
                self._logSch('%s now %s' %
                             (fromField, t['_calc_' + fromField]))
        ...
        def _schedule_task_asap(t):
            ...
            def _wrap_asap_day(s):
                if self.pm.isStartOfDay(
                        s[0] - timedelta(hours=options['hoursPerDay'])):
                    # Move ahead to the start of the next day
        ...
        def serialSGS(scheduleFunction,
            ...
            while unscheduled and eligible:
                ...
                if tid in unscheduled:
                    ...
                    self.env.log.debug(
                        'Could not remove %s from unscheduled list' % tid)
                    self.env.log.debug(' unscheduled:%s' % unscheduled)
        ...
        if options.get('schedule') == 'asap':
            ...
            milestoneIDs = [tid for tid in ticketsByID.keys()
                            if self.pm.isTracMilestone(ticketsByID[tid]) and
                            not self.pm.finish(ticketsByID[tid])]
...
class TicketRescheduler(Component):
    ...
    def _affectsSchedule(self, ticket, old_values):
        ...
        if 'status' in old_values:
            ...
            if (old_values['status'] == 'closed' or
    ...
    def spliceGraph(self, tickets, ticket, old_values):
        ...
        if relationshipChanged:
            ...
            if linkFieldNames['parent'] in old_values:
                ...
                parent = \
                    ticketsByID[old_values[[linkFieldsName['parent']]]]
                parent['children'] = \
                    [cid for cid in parent['children'] if cid != ticket.id]
                # Add ticket to children of new parent
                parent = \
                    ticketsByID[ticket[linkFieldsNames['parent']]]
