...
class RipeStore(object):
    ...
    def delete(cls, env, col1, db=None):
        ...
        cursor = db.cursor()
        cursor.execute("DELETE FROM ripe_store WHERE col1=%s;", (col1,))
    ...
    def insert(cls, env, col1, db=None):
        ...
        cursor = db.cursor()
        cursor.execute("INSERT INTO ripe_store (col1, ) VALUES (%s,);",
                       (col1, ))
        id = db.get_last_id(cursor, 'ripe_store')
    ...
    def get(cls, env, db=None):
        ...
        cursor.execute("SELECT col1 FROM ripe_store ORDER BY col1;")
