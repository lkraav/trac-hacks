...
class WikiToPdfOutput(Component):
    ...
    def process_wikitopdf(self, req, format, title, subject, pages, version,
        ...
        titlefile = title_template and \
                    self.get_titlepage(title_template, title, subject, date,
                                       version) or None
    ...
    def _page_to_file(self, header, req, pagename):
        ...
        page = wiki_to_pdf(WikiPage(self.env, pagename).text, self.env, req,
                           base_dir, codepage)

        hfile, hfilename = tempfile.mkstemp('wikitopdf')
