...
class XsdPlotMacro(WikiMacroBase):
    """
    Plot the data structure of an XMS schema file into an SVG/PNG-image; insert result into a wikipage or ticket.

    Requires `convert` from [http://www.imagemagick.org ImageMagick], `convert` may fail on specific files.

    The code is a rip-off of PdfImg. Thanks to https://trac-hacks.org/wiki/ursaw
    ...
    def expand_macro(self, formatter, name, content):
        ...
        if  not os.access('%s/%s.png' % (self.images_folder, self.filenamehash), os.F_OK) or not self.cache:
            ...
            cmd= "convert -density 400 SVG:\"%s\" -limit area 0  -scale %s PNG:\"%s/%s.png\"" \
                 % (self.svginput , self.width, self.images_folder, self.filenamehash)
            ret = os.system(cmd)

            self.env.log.info("convert command: %s, result = %s", cmd , ret)
    ...
    def parse_trac(self,filespec):
        ...
        if len(parts) == 3:  # realm:id:attachment-filename
            ...
            if parts[0] in browser_links:   # source:path
                ...
                xsd2svg(svnfile.name, outfile=self.svginput)
    ...
    file.preurl  = http://example.com/entrydir
        ...
        if not file_prepath :
            raise TracError ('Cant use Resource \'file:\' without configuration for xsdplot->file.prepath')
        ...
        xsd2svg(xsdinput, outfile=self.svginput)
