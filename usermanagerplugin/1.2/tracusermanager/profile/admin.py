...
from trac.admin.api import IAdminPanelProvider, TracError
...
class UserProfileFieldsAdminPage(Component):
    ...
    def get_admin_panels(self, req):
        if req.perm.has_permission('TRAC_ADMIN'):
            yield ('accounts', _('Accounts'), 'xum_profile_fields',
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        cfadmin = {
            'SUPPORTED_TYPES':
                UserProfileManager(self.env).SUPPORTED_FIELD_TYPES
        }

        # Detail view?
        if path_info:
            currentcf = UserProfileManager(self.env)\
                .get_user_profile_fields(False).get(path_info, False).copy()
            if not currentcf:
                raise TracError(
                    "Profile field %s does not exist." % path_info)

            if req.method == 'POST':
                if req.args.get('save'):
                    cfdict = _field_from_req(self, req)
                    UserProfileManager(self.env)\
                        .update_user_profile_field(cfdict)
                    req.redirect(req.href.admin(cat, page))
                elif req.args.get('cancel'):
                    req.redirect(req.href.admin(cat, page))

            if 'options' in currentcf:
                optional_line = ''
            ...
            if req.method == 'POST':
                ...
                if req.args.get('add') and req.args.get('name'):
                    cfdict = _field_from_req(self, req)
                    UserProfileManager(self.env)\
                        .update_user_profile_field(cfdict, create=True)
                    req.redirect(req.href.admin(cat, page))
                ...
                # Remove Custom Field
                    ...
                    for name in sel:
                        UserProfileManager(self.env)\
                            .delete_user_profile_field({'name': name})
                    ...
                    cf = UserProfileManager(self.env).get_user_profile_fields(
                        False)
                    for cur_cf_name, cur_cf in cf.items():
                        cur_cf['order'] = order[cur_cf_name]
                        UserProfileManager(self.env)\
                            .update_user_profile_field(cur_cf)
                    req.redirect(req.href.admin(cat, page))

            cf_list = []
            for item, attributes in UserProfileManager(self.env)\
                    .get_user_profile_fields(False).items():
                attributes['href'] = req.href.admin(cat, page,
                                                    attributes['name'])
                cf_list.append(attributes)

            cfadmin['customfields'] = \
                sorted(cf_list, lambda x, y: x['order'] - y['order'])
