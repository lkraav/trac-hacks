...
from tracusermanager.admin import IUserListCellContributor, \
                    ...
                    if user.save():
                        data['messages'].append(
                            _("Successfully removed %(user)s's picture.",
                              user=user.username))
            if 'um_profile_update' in req.args:
                    ...
                    if 'um_profile_%s' % field in req.args:
                        if data['um_profile_fields'][field]['type'] == 'file':
                            ...
                            if user_file_new != user_file_old:
                                ...
                                == 'multichecks':
                            user[field] = '|'.join(req.args.getlist(
                                'um_profile_%s' % field))
                        else:
                            user[field] = req.args.get('um_profile_%s'
                                                       % field)
                    elif data['um_profile_fields'][field]['type'] == \
                            'multichecks':
