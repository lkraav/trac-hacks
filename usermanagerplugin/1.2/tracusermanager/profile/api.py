...
class UserProfileManager(Component):
    ...
    def __init__(self):
        self.fields = {
            'name': dict(name='name', type='text', label=_("Name"), value='',
                         order=-3, cols=20),
            'email': dict(name='email', type='text', label=_("Email"),
                          value='', order=-2, cols=20),
            'role': dict(name='role', type='text', label=_("Role"), value='',
                         order=-1, cols=20),
            'picture_href': dict(name='picture_href', type='file',
                                 label=_("Picture"), value='', order=0)}

        for field, attributes in \
                get_custom_fields_config(self.config,
                                         self.CONFIG_SECTION_NAME).items():
    ...
    def get_user_profile_fields(self, all=True, ignore_internal=False):
        if all:
            if ignore_internal:
                return dict(
                    filter(lambda x: int(x[1].get('internal', '0')) == 0,
                           self.fields.items()))
            return self.fields
        else:
            return dict(filter(lambda x: not x[0] in ['name', 'email', 'role',
                                                      'picture_href'],
                               self.fields.items()))
    ...
    def update_user_profile_field(self, field, create=False):
        ...
        field is a dictionary with the following possible keys:
            ...
            internal = if True field will be visible only on the admin page
        """
        ...
        if not ('name' in field and 'type' in field and 'label' in field):
            raise TracError("Custom field needs at least a name, type and "
                            "label.")

        # Only alphanumeric characters (and [-_]) allowed for custom fieldname
        match_len = re.search("[a-z0-9-_]+", field['name']).span()
        name_len = len(field['name'])
        if match_len[1] - match_len[0] != name_len:
            raise TracError("Only alphanumeric characters allowed for "
        ...
        # Set the field label
        env.config.set(self.CONFIG_SECTION_NAME, field['name'] + '.label',
                       field['label'])

        # Set default value if it exist in dictionary with value, else
        # remove it if it exists in config
        if 'value' in field and field['value']:
            env.config.set(self.CONFIG_SECTION_NAME, field['name'] + '.value',
                           field['value'])
        elif env.config.get(self.CONFIG_SECTION_NAME,
                            field['name'] + '.value'):
            env.config.remove(self.CONFIG_SECTION_NAME,
                              field['name'] + '.value')

        # If select or radio set options, or remove if it exists and
        # field no longer need options
        if field['type'] in ['select', 'multichecks']:
            if 'options' not in field or field['options'] == []:
                raise TracError(_("No options specified for %(field)s field",
                                  field=field['type']))
        ...
        # Set defaults for textarea if none is specified, remove settings
        # if no longer used
        env.config.set(self.CONFIG_SECTION_NAME, field['name'] + '.cols',
                       field['cols'])
        env.config.set(self.CONFIG_SECTION_NAME, field['name'] + '.rows',
                       field['rows'])

        # internal ?
        env.config.set(self.CONFIG_SECTION_NAME, field['name'] + '.internal',
                       field.get('internal', 0))

        # Set sort setting if it is in customfield dict, remove if not present
        if create:
    ...
    def delete_user_profile_field(self, field, config_save=True):
        ...
        # Need to redo the order of fields that are after the field to be
        # deleted
        order_to_delete = env.config.getint(self.CONFIG_SECTION_NAME,
                                            field['name'] + '.order')
        ...
        for profile_field_name, profile_field in cfs.items():
            if profile_field['order'] > order_to_delete:
                env.config.set(self.CONFIG_SECTION_NAME,
                               profile_field['name'] + '.order',
                               profile_field['order'] - 1)

        # Remove any data for the custom field (covering all bases)
        env.config.remove(self.CONFIG_SECTION_NAME, field['name'])
        for attribute_name in ['.name', '.type', '.label', '.value',
                               '.options', '.cols', '.rows', '.order',
                               '.internal']:
            env.config.remove(self.CONFIG_SECTION_NAME,
    ...
    def get_uploaded_file_href(self, req, user, field, req_field):
        ...
        if upload is None or \
                not hasattr(upload, 'filename') or \
                not upload.filename:
            return user[field]
        ...
        return req.href('/'.join(
            ['raw-attachment', 'wiki', self.attachments_wikiPage,
             attachment.filename]))
    ...
    def remove_user_file(self, file_href):
        ...
        """
        try:
            Attachment(self.env, 'wiki', self.attachments_wikiPage,
                       filename=unicode_unquote(
                           file_href.split('/')[-1])).delete()
            return True
        except Exception, e:
            out = StringIO()
            ...
            self.log.error("%s: %s\n%s", self.__class__.__name__, e,
                           out.getvalue())
            raise TracError(_("Unable to remove file. %(file)s.",
                              file=file_href))
