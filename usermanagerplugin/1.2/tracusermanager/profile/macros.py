...
from tracusermanager.profile.api import (
    IUserProfilesListMacroCellContributor, UserProfileManager)
...
[[UserProfilesList(|class=someCSS_Class,
    ...
    def expand_macro(self, formatter, name, content, args=None):
        ...
        if user_profile_templates:
            data['user_profiles'] = \
                UserManager(env).search_users(user_profile_templates)
        else:
...
class MacroArguments(dict):
    ...
    def __init__(self, arguments):
        super(MacroArguments, self).__init__()
