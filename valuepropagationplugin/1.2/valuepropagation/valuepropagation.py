...
class ValuePropagationPlugin(Component):
    """Update values in other tickets based on ticket changes.

    For example, with Subtickets plugin and Timing and Estimation plugin
    in place, a parent ticket's estimate may be the sum of its children's
    estimates. This plugin can update the parent when any children change.
    ...
    def __init__(self):
        ...
        def _method_min(ticket, old_values, f, t, old_other, new_other,
                        options):
            other_v = float(new_other[t])
            my_v = float(ticket[f])
            if my_v < other_v:
                new_other[t] = str(my_v)

        # Other's value is the max of my value and other's old value
        def _method_max(ticket, old_values, f, t, old_other, new_other,
                        options):
            other_v = float(new_other[t])
            my_v = float(ticket[f])
            if my_v > other_v:
        ...
        def _get_default(ticket, options):
            self.log.debug("Getting default for %s", ticket.id)
            dft = ''
            for n, v in options:
                self.log.debug("Processing (%s,%s)", n, v)
                ...
                if p[1] == 'default':
                    ...
                    link = p[0]
                    field = p[1]
                    self.log.debug("link:%s, field:%s", link, field)
                    if link == 'id':
                        other = Ticket(self.env, ticket.id)
                    else:
                        other = Ticket(self.env, ticket[link])
                    self.log.debug("other is %s", other.id)
                    dft = other[field]
                    self.log.debug("dft set to %s", dft)
            return dft

        # When my value changes, replace my old value at the end of
        # other's value.
        def _method_suffix(ticket, old_values, f, t, old_other, new_other,
                           options):
            v = old_other[t]
            if v is None or v == '':
        ...
        def _method_prefix(ticket, old_values, f, t, old_other, new_other,
                           options):
            # First remove my old value from the beginning of the oldOther's
            # When creating a ticket, there's no old value to remove.
            if f in old_values and old_values[f]:
                other_v = old_other[t].split(',')
                my_old_v = old_values[f].split(',')
                other_v = other_v[len(my_old_v):]
                old_other[t] = ','.join(other_v)

            # Next, add my value to the beginning of newOther's
            other_v = new_other[t].split(',')
            my_v = ticket[f].split(',')
            other_v = my_v + other_v
            new_other[t] = ','.join(other_v)

        self.methods = {'sum': _method_sum, 'min': _method_min,
                        'max': _method_max, 'suffix': _method_suffix,
        ...
        self.types = {'link': self._handle_link, 'query': self._handle_query,
    ...
    def _handle_link(self, r, ticket, old_values):
        ...
        if f is None:
            ...
            if ticket[f]:
                ...
                if f in old_values:
                    old_other_i_d = old_values[f]
                # Otherwise, the old other is the same as the new
                else:
                    old_other_i_d = new_other_id
                self._propagate(r, ticket, old_values, old_other_i_d,
                                new_other_id)
            else:
    ...
    def _propagate(self, relation, ticket, old_values, old_other_id,
        ...
        fields = self.config.getlist('value_propagation',
                                     '%s.fields' % relation)
        for p in fields:
            ...
            if len(old_values) == 0 or \
                ...
                if f in self.enums:
                    ...
                    if self.enums[f] in old_values:
                        old_values[f] = self.p_values[
                            old_values[self.enums[f]]]

                # Call the method for the propagated field
                method = self.config.get('value_propagation',
                                         '%s.method.%s' % (relation, f))
                if method in self.methods:
                    options = self.config.options('value_propagation')
                    options = [(n, v) for n, v in options if
                               n.startswith(relation + '.')]
                    self.methods[method](ticket, old_values, f, t,
                                         old_other, new_other, options)
                else:
                    self.log.debug("No %s method found", method)
        ...
        for n, v in options:
            ...
            if len(p) == 2 and p[1] == 'type':
                if v in self.types:
                    ...
                    self.log.debug("%s has an unknown type '%s'", n, v)

    def ticket_deleted(self, ticket):
        self.log.debug("valuePropagation noted ticket %d deleted", ticket.id)
