...
class WorlLogRPC(Component):
    """Interface to the WorklogPlugin."""
    ...
    def startWork(self, req, ticket):
        """Start work on a ticket. Returns the string 'OK' on success
        or an explanation on error (requires authentication).
        """
    ...
    def stopWork(self, req, comment='', stoptime=None):
        """Stops work. Returns the string 'OK' on success or an explanation
        on error (requires authentication, stoptime is seconds since epoch).
        """
        mgr = WorkLogManager(self.env, self.config, req.authname)
    ...
    def getLatestTask(self, req, username=None):
        """Returns a structure representing the info about the latest
        task.
        """
        if username:
    ...
    def getActiveTask(self, req, username=None):
        """Returns a structure representing the info about the active
        task (identical to getLatestTask but does not return anything
    ...
    def whoIsWorkingOn(self, req, ticket):
        """Returns the username of the person currently working on the
        given ticket.
        """
        mgr = WorkLogManager(self.env, self.config, req.authname)
    ...
    def whoLastWorkedOn(self, req, ticket):
        """Returns the username of the person last worked on the
        given ticket.
        """
        mgr = WorkLogManager(self.env, self.config, req.authname)
